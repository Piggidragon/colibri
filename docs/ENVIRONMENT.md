...
| `V4_KV` | `native` | Main V4 KV row format. `native` stores the model's existing FP8+E8M0 values and BF16 RoPE tail without changing their decoded bits; `f32` keeps the diagnostic legacy layout. `turbo4`, `turbo3`, and `turbo2` apply increasingly aggressive lossy TurboQuant compression and require a head dimension divisible by 128. Unsupported geometry and malformed values warn and fall back to `native`. |
| `V4_KV_INDEX` | `native` | Lightning Indexer KV row format, selected separately because lossy compression can change the top-k token set. `native` losslessly packs the existing FP4+E8M0 values; `f32` keeps the diagnostic legacy layout. `turbo4`, `turbo3`, and `turbo2` are opt-in and require an index-head dimension divisible by 128; unsupported geometry and malformed values warn and fall back to `native`. For the 128-value index row, `turbo4` saves only 2 B versus `native` (66 B instead of 68 B), so use `turbo3` or `turbo2` when accepting lossy index compression for a material saving. |
| `V4_VRAM` | `0` | Set to `1` in a `CUDA=1` build to mirror the encoded V4 attention KV in VRAM and run sparse MLA attention there when `V4_FLASH=1`; `V4_FLASH=0` still selects the CPU two-pass reference path for A/B diagnostics. When the complete FP8 dense tier fits, the planner also uploads its row-major weights and f32 scales, releases those host buffers, and dispatches the attention projections, indexer query and shared expert on CUDA. The measured full checkpoint moves 5.456 GiB while its 0.810 GiB BF16/f32/i64 remainder stays resident in RAM. The BF16 output head is loaded directly to CUDA in 32 MiB chunks and its argmax runs there; full DSpark additionally uploads its FP8 backbone lazily. A head upload that does not fit is replanned on RAM; a later DSpark upload failure disables drafting for that engine rather than reviving its host tensors after the target cache was sized. A dense upload failure disables dense residency, releases every layer already uploaded, and returns to CPU streaming. A dense *compute* failure means VRAM is exhausted and the host FP8 buffers are already gone, so the path latches off with one `v4_dense warning=…` line and the call fails rather than re-reading the tensor from the checkpoint on every projection: re-run with `V4_VRAM=0`. Since Phase 08, the attention KV device mirror is only attempted when the unified VRAM tier planner (`coli_v4_vram_tier_plan`, highest priority of the four tiers) budgeted room for it — previously every session tried it unconditionally whenever CUDA was ready, regardless of how much dense/head/DSpark had already claimed. The plan still keeps the host copy alongside a resident device mirror (Phase 05's "host shadow"); dropping it once the device mirror is confirmed live is unclaimed follow-up work, not part of Phase 08. CUDA initialization, KV allocation/upload, or attention-kernel failures otherwise retain the Phase-05 fallback behavior. Malformed values keep the default off. CUDA reductions are equivalent only within the harness tolerances, not bit-identical to the host's sequential computation; token-identical checkpoint runs are evidence, not a guarantee. |
| `V4_VRAM_RESERVE_MB` | free VRAM / 8, clamped to 256–1024 MiB | VRAM the dense tier may not claim, so that the per-session attention KV mirror, the kernel scratch and allocator granularity still have somewhere to go. Only consulted in a `CUDA=1` build with `V4_VRAM=1`. An explicit value overrides the computed default and is clamped to 0–16384 MiB; malformed text falls back to the default. Setting `0` restores the pre-Phase-06 behaviour of admitting dense against the whole free VRAM, which can make every later `v4_cuda_kv_alloc` fail and silently disable Phase-05 CUDA attention. Since Phase 08, dense admission is decided against this value plus the reserved attention-KV device budget (`coli_v4_kv_device_bytes`); the `ram_tiers` line's `vram=…(reserve=…)` therefore reports the combined figure, not this variable alone. The Phase-08 `vram_tiers` line's own `reserve=…` still reports just this variable's value. |
| `V4_VRAM_LIMIT_MB` | off (uses the real free VRAM) | Clamps what the Phase-08 planner believes is free, so its degrade path can be exercised without owning a smaller card. Only ever lowers the reported free bytes; a value at or above the real free amount, or malformed text, is a no-op. `0` is a valid clamp-down value, not malformed, and forces the reported free bytes to zero -- a way to exercise the fully-degraded RAM path without unsetting `V4_VRAM`. Applied before `V4_VRAM_RESERVE_MB`'s own clamp, so a very small limit can leave a zero budget rather than underflowing. |
| `V4_VRAM_FAIL_AT` | off | `kv`, `dense`, `head`, or `dspark`: forces that tier's device upload to fail once, so the Phase-08 runtime-degrade path (the plan admitted VRAM, the upload fails anyway) can be tested without a real OOM. Any other value is a no-op. Compiled only under `COLI_V4_TEST_HOOKS`; outside that build a constant-folded stub makes the check free and unreachable via the environment in a release build. |
...
| `V4_PREFILL_CHUNK` | `0` (off) | Phase 14: bounds the two V4 prefill activation buffers while preserving one continuous KV/Compressor/Indexer history. `0` keeps the full-prompt path. Positive values are clamped to 64–65536 tokens and rounded down to a multiple of 64; malformed, negative, and overflowing values fall back to `0`. The effective value is capped by the session prompt capacity and logged as `v4_prefill`. Chunking is target-only during prefill; DSpark speculation begins only at decode. |
| `V4_OMP_CORES` | `all` physical cores | V4-only automatic OpenMP team. `perf` uses one worker per detected P-core (six on the i5-13400F); it is opt-in until its hardware A/B is recorded. `all` keeps the prior physical-core count; a numeric value is clamped to 1…logical CPUs. Malformed text falls back to `all`. `OMP_NUM_THREADS` is the higher-priority explicit override, and `COLI_NO_OMP_TUNE` disables automatic selection. |
| `COLI_MODEL_MIRROR` | unset | Phase 10 V4-only opt-in: `;`/`,`-separated directories containing byte-identical safetensors shards. Without it V4 opens no replica, starts no probe and uses its unchanged single-SSD reader. With it, each routed expert deterministically selects one replica; a complete mirror additionally splits weights of at least 4 MiB into 4-KiB-aligned weighted stripes. Missing shards on a partial mirror fall back to the primary for that expert. |
| `COLI_DISK_WEIGHTS` | unset (`1,1,…` for V4) | Positive comma-separated primary/mirror weights for the V4 mirror route and stripe split, for example `2,1` on this Gen4+Gen3 setup. The V4 path deliberately does not run a startup probe; Plan 15 supplies the physical-drive measurement and may document a measured override. Invalid or mismatched input falls back to equal weights without changing model output. |
| `V4_PIN_SLOTS` | compile-time `COLI_V4_MAX_PIN_SLOTS_PER_LAYER` (`16` in the shipped build) | Phase 12: absolute upper bound on autopinned experts per layer, replacing what used to be a compile-time-only ceiling. Always clamped into `[0, available_pins]` (`slots_per_layer` minus the reserved minimum-6 LRU floor); a value above `available_pins` is silently capped, not an error. Wins over `V4_PIN_FRACTION` only when it parses cleanly; malformed or negative text falls through to `V4_PIN_FRACTION` if that is set, and only to the compile-time default if it is not. |
| `V4_PIN_FRACTION` | unset | Phase 12: alternative to `V4_PIN_SLOTS`, expressed as a `0..1` share of `available_pins` instead of an absolute count, rounded to the nearest integer. Out-of-range (outside `0..1`) or malformed text falls back to the compile-time default, same as `V4_PIN_SLOTS`. `V4_PIN_SLOTS` is applied on top and wins, but only if it parses cleanly -- a garbage `V4_PIN_SLOTS` with a valid `V4_PIN_FRACTION` set uses the fraction, not the compile-time default. |
| `V4_PIN_RAMP_REQUESTS` | compile-time `COLI_V4_PIN_RAMP_REQUESTS` (`24` in the shipped build) | Phase 12: requests-per-layer divisor that ramps a fresh policy's active pin count up gradually instead of pinning all of `V4_PIN_SLOTS` from the first request. The ramp starts at a hardcoded floor of 4 active pins and is a no-op whenever `V4_PIN_SLOTS` resolves to 4 or fewer. Whether the ramp runs at all is decided once at store-open time from the on-disk usage history (≥5000 requests recorded there means the ramp is skipped for the rest of the session); within a running session only the ramp catching up to the full pin count ends it. `0` disables the ramp (every pin active immediately). Malformed or negative text falls back to the compile-time default. |
