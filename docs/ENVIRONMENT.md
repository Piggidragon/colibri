...
| `V4_VRAM` | `0` | Set to `1` in a `CUDA=1` build to keep the encoded V4 attention KV in VRAM and run sparse MLA attention there. CUDA initialization, allocation, upload, or kernel failures fall back to the CPU path; malformed values keep the default off. |
