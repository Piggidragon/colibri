...
| `V4_PIN_SLOTS` | compile-time `COLI_V4_MAX_PIN_SLOTS_PER_LAYER` (`16` in the shipped build) | Phase 12: absolute upper bound on autopinned experts per layer, replacing what used to be a compile-time-only ceiling. Always clamped into `[0, available_pins]` (`slots_per_layer` minus the reserved minimum-6 LRU floor); a value above `available_pins` is silently capped, not an error. Malformed or negative text falls back to the compile-time default. Wins over `V4_PIN_FRACTION` if both are set. |
| `V4_PIN_FRACTION` | unset | Phase 12: alternative to `V4_PIN_SLOTS`, expressed as a `0..1` share of `available_pins` instead of an absolute count, rounded to the nearest integer. Out-of-range (outside `0..1`) or malformed text falls back to the compile-time default, same as `V4_PIN_SLOTS`. Ignored if `V4_PIN_SLOTS` is also set. |
| `V4_PIN_RAMP_REQUESTS` | compile-time `COLI_V4_PIN_RAMP_REQUESTS` (`24` in the shipped build) | Phase 12: requests-per-layer divisor that ramps a fresh policy's active pin count up gradually instead of pinning all of `V4_PIN_SLOTS` from the first request, until either usage history seeds (≥5000 requests) or the ramp catches up. `0` disables the ramp (every pin active immediately). Malformed or negative text falls back to the compile-time default. |
