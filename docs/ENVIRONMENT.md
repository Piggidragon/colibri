...
| `V4_VRAM` | `0` | Set to `1` in a `CUDA=1` build to mirror the encoded V4 attention KV in VRAM and run sparse MLA attention there when `V4_FLASH=1`; `V4_FLASH=0` still selects the CPU two-pass reference path for A/B diagnostics. Phase 05 retains the host KV for an immediate CPU fallback, so this switch alone does not free RAM; exclusive tier ownership belongs to the Phase-08 planner. CUDA initialization, allocation, upload, or kernel failures fall back to the CPU path; a rejected kernel launch falls back for that token only and the next one retries on the device, while an initialization, allocation or upload failure drops the VRAM mirror for the rest of the process. Either way the `v4_cuda warning=…; continuing-on-cpu` notice is printed once per process. Malformed values keep the default off. Device `expf` plus tree-reduced dot products are equivalent only within the harness tolerances and cosine threshold, not bit-identical to the host's sequential computation; token-identical checkpoint runs are evidence, not a guarantee. |
