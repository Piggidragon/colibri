...
| `V4_VRAM` | `0` | Set to `1` in a `CUDA=1` build to mirror the encoded V4 attention KV in VRAM and run sparse MLA attention there. Phase 05 retains the host KV for an immediate CPU fallback, so this switch alone does not free RAM; exclusive tier ownership belongs to the Phase-08 planner. CUDA initialization, allocation, upload, or kernel failures fall back to the CPU path; malformed values keep the default off. |
