...
static int compare_outputs(const float *reference, const float *flash,
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        float relative = scale > 0.0f
            ? fabsf(reference[i] - flash[i]) / scale : 0.0f;
        if (relative >= 1e-4f) {
            fprintf(stderr,
                    "%s[%zu]: reference=%g flash=%g relative=%g\n",
                    label, i, reference[i], flash[i], relative);
            return 1;
        }
...
static int compare_random_case(int topk, int padded) {
    ...
    for (size_t i = 0; i < (size_t)HEADS * HEAD_DIM; i++)
        queries[i] = fabsf(random_float());
    for (int i = 0; i < HEAD_DIM; i++) kv[i] = 1.0f;
    ...
    int failed = coli_v4_sparse_attention_ref(
        ...
        scale) || coli_v4_flash_attention_ref(
    ...
    char label[64];
    snprintf(label, sizeof(label), "topk=%d padded=%d", topk, padded);
    if (!failed)
        failed = compare_outputs(reference, flash,
                                 (size_t)HEADS * HEAD_DIM, label);
    free(compressed_indices); free(window_indices); free(indices);
    free(flash); free(reference); free(queries); free(kv);
    return failed;
}
...
static int test_random_cases(void) {
    ...
    for (size_t i = 0; i < sizeof(topks) / sizeof(topks[0]); i++) {
        if (compare_random_case(topks[i], 0)) return 1;
        if (topks[i] > 1 && compare_random_case(topks[i], 1)) return 1;
    }
    return 0;
}
...
static int test_edge_cases(void) {
    ...
    if (coli_v4_flash_attention_ref(output, query, window, 2, NULL,
    ...
    if (coli_v4_flash_attention_ref(output, dominant_query, dominant_values, 2,
                                    NULL, dominant_indices, NULL, 0, &sink,
                                    1, 2, 1.0f))
        return 1;
    ...
    if (coli_v4_flash_attention_ref(output, one_query, one_value, 1, NULL,
...
static int test_switch(void) {
    ...
    if (coli_v4_attention_two_source_ref(
            actual, query, values, 2, NULL, indices, NULL, 0,
    ...
    if (coli_v4_attention_two_source_ref(
        ...
        coli_v4_flash_attention_ref(
            flash, query, values, 2, NULL, indices, NULL, 0,
...
static int benchmark_long_context(void) {
    ...
    if (!values || !staged || !queries || !output || !indices ||
        !window_indices || !compressed_indices) return 1;
    ...
    for (int repeat = 0; repeat < REPEATS; repeat++)
        if (coli_v4_flash_attention_ref(
                ...
                window_indices, compressed_indices, SELECTED,
...
int main(int argc, char **argv) {
    if (test_random_cases() || test_edge_cases() || test_switch()) {
